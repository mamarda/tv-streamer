...
def mjpeg_generator(hls_url: str, width: int = 640, fps: int = 20, jpeg_quality: int = 6) -> Generator[bytes, None, None]:
    """
    Pull HLS, transcode to JPEG frames, and yield a well-formed
    multipart/x-mixed-replace stream (~fps).
    """
    cmd = [
        "ffmpeg",
        "-nostdin",
    ...
    # If your source needs a Referer header, replace the two args below:
    # cmd[cmd.index("-user_agent"):cmd.index("-user_agent")+2] = [
    ...
    try:
        while True:
            ...
            if not chunk:
                ...
                break
            buf.extend(chunk)
            # find JPEG End-Of-Image marker 0xFF 0xD9
            while True:
